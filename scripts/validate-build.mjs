...
const requiredFiles = [
  ...
  'docs/V235.12.7-HORDE-VISIBILITY-HOTFIX.md',
...
if (!runtimeSource.includes("version:ATM_CONFIG?.build?.version||'v235.12.7'")) errors.push('Missing v235.12.7 display build marker.');
if (!runtimeSource.includes("name:ATM_CONFIG?.build?.name||'Horde Visibility Hotfix'")) errors.push('Missing v235.12.7 display fallback.');
...
if (!configSource.includes("version: 'v235.12.7'")) errors.push('js/config.js is not marked v235.12.7.');
...
if (!serviceWorkerSource.includes("atm-town-shell-v235.12.7") || !serviceWorkerSource.includes('/js/zombie-outbreak.js') || !serviceWorkerSource.includes('/js/prop-hunt.js') || !serviceWorkerSource.includes('/assets/maps/town/foreground/day/assets_05_00.webp') || !serviceWorkerSource.includes('/assets/maps/town/foreground/day/assets_18_00.webp') || !serviceWorkerSource.includes('/js/runtime/neon-racer.js') || !serviceWorkerSource.includes('/js/live-chat.js')) errors.push('v235.12.7 PWA shell cache/module registration is missing.');
...
if (!runtimeSource.includes('const HORDE_NIGHTFALL=') || !runtimeSource.includes('currentTownNightAlpha=Math.max(getTownNightAlpha(getSharedTownTimeMs()),hordeNightfallAlpha*HORDE_NIGHTFALL.nightMix)')) errors.push('v235.9.4+ Horde event does not apply the outdoor nightfall mix.');
...
if (errors.length) {
  console.error(`ATM Town v235.12.7 build validation failed with ${errors.length} issue(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log('ATM Town v235.12.7 build validation passed.');
