...
if (!runtimeSource.includes("version:ATM_CONFIG?.build?.version||'v235.10'")) errors.push('Missing v235.10 display build marker.');
if (!runtimeSource.includes("name:ATM_CONFIG?.build?.name||'You Are ATM Entitlements'")) errors.push('Missing v235.10 display fallback.');
...
if (!configSource.includes("version: 'v235.10'")) errors.push('js/config.js is not marked v235.10.');
...
if (!serviceWorkerSource.includes("atm-town-shell-v235.10") || !serviceWorkerSource.includes('/js/zombie-outbreak.js')) errors.push('v235.10 PWA shell cache/module registration is missing.');
...
if (!html.includes('id="lockerYouAreAtmCount"') || !html.includes('id="lockerNftUnlockCount"') || !html.includes('LOCKER v6 · YATM ENTITLEMENTS · v235.10')) errors.push('v235.10 Locker entitlement summary/UI markers are missing.');
...
if (errors.length) {
  console.error(`ATM Town v235.10 build validation failed with ${errors.length} issue(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log('ATM Town v235.10 build validation passed.');
