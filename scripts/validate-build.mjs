...
if (!runtimeSource.includes("version:ATM_CONFIG?.build?.version||'v235.11.6'")) errors.push('Missing v235.11.4 display build marker.');
if (!runtimeSource.includes("name:ATM_CONFIG?.build?.name||'Checkout UX Rebuild + Entry Stability'")) errors.push('Missing v235.11.4 display fallback.');
...
if (!configSource.includes("version: 'v235.11.6'")) errors.push('js/config.js is not marked v235.11.4.');
...
if (!serviceWorkerSource.includes("atm-town-shell-v235.11.6") || !serviceWorkerSource.includes('/js/zombie-outbreak.js')) errors.push('v235.11.4 PWA shell cache/module registration is missing.');
...
if (!html.includes('id="lockerYouAreAtmCount"') || !html.includes('id="lockerNftUnlockCount"') || !html.includes('LOCKER v7 · OWNED ASSETS ONLY · v235.11.6')) errors.push('v235.11 Locker ownership UI markers are missing.');
if (!html.includes('id="attributeStoreButton"') || !html.includes('id="attributeStorePanel"') || !html.includes('id="attributeStoreCharacters"') || !html.includes('ATTRIBUTE STORE v1.4 · v235.11.6')) errors.push('v235.11.6 Attribute Store UI markers are missing.');
if (!html.includes('id="attributeStorePaymentCategories"') || !html.includes('id="attributeStoreCryptoAssets"') || !html.includes('XRPL MAINNET') || !html.includes('World-event funding and rewards remain on Testnet')) errors.push('v235.11.6 CASH/CRYPTO payment UI markers are missing.');
if (!html.includes('id="attributeStoreCheckoutSheet"') || !html.includes('id="attributeStoreCheckoutReviewButton"') || !html.includes('id="attributeStoreCheckoutBackButton"')) errors.push('v235.11.6 dedicated checkout sheet controls are missing.');
if (!html.includes('.attributeStoreCheckoutSheet{position:absolute;inset:0') || !html.includes('contain:inline-size') || !html.includes('.attributeStoreCryptoAssets[hidden],.attributeStoreCashInfo[hidden]{display:none!important}') || !html.includes('.attributeStorePaymentCategories,.attributeStoreCryptoAssets{display:flex!important;flex-direction:column')) errors.push('v235.11.6 checkout width containment/mobile stacking is missing.');
if (!gameRuntimeParts[0].includes('function attributeStoreCheckoutOpen()') || !gameRuntimeParts[0].includes('function attributeStoreCheckoutClose()') || !gameRuntimeParts[0].includes('attributeStoreState={open:false,checkoutOpen:false')) errors.push('v235.11.6 checkout sheet runtime state is incomplete.');
...
if (!html.includes('id="attributeStoreCloseButton"') || !html.includes('position:absolute;top:10px;right:10px') || !html.includes('aria-label="Close Attribute Store"')) errors.push('v235.11.6 persistent Attribute Store close control is missing.');
...
if (errors.length) {
  console.error(`ATM Town v235.11.6 build validation failed with ${errors.length} issue(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log('ATM Town v235.11.6 build validation passed.');
