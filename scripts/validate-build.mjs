...
if (!html.includes("version:ATM_CONFIG?.build?.version||'v231.2'")) errors.push('Missing v231 display build marker.');
if (!html.includes("name:ATM_CONFIG?.build?.name||'XRPL NFT IPFS Compatibility'")) errors.push('Missing v231 display build name.');
...
if (!configSource.includes("version: 'v231.2'")) errors.push('js/config.js is not marked v231.');
...
if (errors.length) {
  console.error(`ATM Town v231.2 build validation failed with ${errors.length} issue(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log('ATM Town v231.2 build validation passed.');
