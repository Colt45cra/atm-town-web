...
if (!runtimeSource.includes("version:ATM_CONFIG?.build?.version||'v235.9'")) errors.push('Missing v235.9 display build marker.');
if (!runtimeSource.includes("name:ATM_CONFIG?.build?.name||'Synchronized Zombie Combat'")) errors.push('Missing v235.9 display fallback.');
...
if (!configSource.includes("version: 'v235.9'")) errors.push('js/config.js is not marked v235.9.');
...
if (!serviceWorkerSource.includes("atm-town-shell-v235.9") || !serviceWorkerSource.includes('/js/zombie-outbreak.js')) errors.push('v235.9 PWA shell cache/module registration is missing.');
...
if (errors.length) {
  console.error(`ATM Town v235.9 build validation failed with ${errors.length} issue(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log('ATM Town v235.9 build validation passed.');
