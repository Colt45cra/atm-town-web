...
if (!runtimeSource.includes("version:ATM_CONFIG?.build?.version||'v235.11.5'")) errors.push('Missing v235.11.4 display build marker.');
if (!runtimeSource.includes("name:ATM_CONFIG?.build?.name||'Mobile Checkout Fit + Entry Stability'")) errors.push('Missing v235.11.4 display fallback.');
...
if (!configSource.includes("version: 'v235.11.5'")) errors.push('js/config.js is not marked v235.11.4.');
...
if (!serviceWorkerSource.includes("atm-town-shell-v235.11.5") || !serviceWorkerSource.includes('/js/zombie-outbreak.js')) errors.push('v235.11.4 PWA shell cache/module registration is missing.');
...
if (!html.includes('id="lockerYouAreAtmCount"') || !html.includes('id="lockerNftUnlockCount"') || !html.includes('LOCKER v7 · OWNED ASSETS ONLY · v235.11.5')) errors.push('v235.11 Locker ownership UI markers are missing.');
if (!html.includes('id="attributeStoreButton"') || !html.includes('id="attributeStorePanel"') || !html.includes('id="attributeStoreCharacters"') || !html.includes('ATTRIBUTE STORE v1.3 · v235.11.5')) errors.push('v235.11.5 Attribute Store UI markers are missing.');
if (!html.includes('id="attributeStorePaymentCategories"') || !html.includes('id="attributeStoreCryptoAssets"') || !html.includes('XRPL MAINNET') || !html.includes('World-event funding and rewards remain on Testnet')) errors.push('v235.11.5 CASH/CRYPTO payment UI markers are missing.');
if (!html.includes('.attributeStorePaymentCategories{display:grid!important;grid-template-columns:minmax(0,1fr) minmax(0,1fr)!important') || !html.includes('.attributeStoreCryptoAssets{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important') || !html.includes('.attributeStorePaymentCategory,.attributeStoreCryptoAsset{display:block!important;width:100%!important;min-width:0!important')) errors.push('v235.11.5 mobile payment controls are not hard-bounded.');
...
if (!html.includes('id="attributeStoreCloseButton"') || !html.includes('position:absolute;top:10px;right:10px') || !html.includes('aria-label="Close Attribute Store"')) errors.push('v235.11.5 persistent Attribute Store close control is missing.');
...
if (errors.length) {
  console.error(`ATM Town v235.11.5 build validation failed with ${errors.length} issue(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log('ATM Town v235.11.5 build validation passed.');
