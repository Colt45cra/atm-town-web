...
const requiredFiles = [
  ...
  'docs/V235.9.3-HORDE-GROUNDING-ANIMATION.md',
  'docs/V235.9.4-HORDE-NIGHTFALL.md',
  'docs/V235.10-YOU-ARE-ATM-ENTITLEMENTS.md',
  'docs/V235.11-ATTRIBUTE-STORE.md',
  ...
  'assets/maps/town/foreground/day/assets_09_00.webp',
  'assets/maps/town/foreground/day/assets_11_00.webp',
...
if (!runtimeSource.includes("version:ATM_CONFIG?.build?.version||'v235.12.6'")) errors.push('Missing v235.12.6 display build marker.');
if (!runtimeSource.includes("name:ATM_CONFIG?.build?.name||'Prop Hunt Polish + Horde Darkness'")) errors.push('Missing v235.12.6 display fallback.');
...
if (!configSource.includes("version: 'v235.12.6'")) errors.push('js/config.js is not marked v235.12.6.');
...
const propHuntSource = await readFile(path.join(root, 'js', 'prop-hunt.js'), 'utf8');
...
if (!serviceWorkerSource.includes("atm-town-shell-v235.12.6") || !serviceWorkerSource.includes('/js/zombie-outbreak.js') || !serviceWorkerSource.includes('/js/prop-hunt.js') || !serviceWorkerSource.includes('/assets/maps/town/foreground/day/assets_05_00.webp') || !serviceWorkerSource.includes('/assets/maps/town/foreground/day/assets_18_00.webp') || !serviceWorkerSource.includes('/js/runtime/neon-racer.js') || !serviceWorkerSource.includes('/js/live-chat.js')) errors.push('v235.12.6 PWA shell cache/module registration is missing.');
...
if (!worldTimeApiSource.includes("action === 'start-prop-hunt'") || !worldTimeApiSource.includes("action === 'tag-prop-hunt'") || !worldEventsClientSource.includes('START PROP HUNT')) errors.push('v235.12.6 Prop Hunt World Event API/control-panel bridge is missing.');
if (!worldEventsServerSource.includes('PROP_HUNT_EVENT_TYPE') || !worldEventsServerSource.includes('startPropHunt') || !worldEventsServerSource.includes('tagPropHuntPlayer')) errors.push('v235.12.6 Prop Hunt server orchestration is missing.');
if (!propHuntSource.includes('drawPlayerOverride') || !propHuntSource.includes('nearestTarget') || !propHuntSource.includes('assets/maps/town/foreground/day/assets_05_00.webp') || !propHuntSource.includes('assets/maps/town/foreground/day/assets_18_00.webp') || !gameRuntimeParts[0].includes('window.ATMPropHunt?.drawPlayerOverride') || !gameRuntimeParts[0].includes('window.ATMPropHunt?.nearestTarget')) errors.push('v235.12.6 Prop Hunt gameplay/render integration is missing.');
if (!gameRuntimeParts[0].includes("items.push({depth:p.drawY+20,type:'remote',id:String(id),p})") || !gameRuntimeParts[0].includes('sessionId:item.id')) errors.push('v235.12.6 Prop Hunt remote-player id scoping hotfix is missing.');
if (!propHuntSource.includes("if (isFound(state.localId)) return 'seeker'") || !propHuntSource.includes('isSeeker(state.localId)') || !worldEventsServerSource.includes('seeker_session_id') || !worldEventsServerSource.includes('Only a Prop Hunt seeker can tag hidden props.')) errors.push('v235.12.6 found-prop-to-seeker flow is incomplete.');
...
if (!gameRuntimeParts[0].includes('const HORDE_NIGHTFALL={fadeInPerSecond:1.9,fadeOutPerSecond:.72,visionInner:66,visionOuter:172,darkness:.965};') || !gameRuntimeParts[0].includes("target.globalCompositeOperation='destination-out'") || !gameRuntimeParts[0].includes('street lamps. The blackout is drawn first')) errors.push('v235.12.6 Horde darkness overhaul is missing.');
...
if (!zombieOutbreakSource.includes('z.dir, z.moving ? 1 : 0, roundNet(z.animClock, 2)') || !zombieOutbreakSource.includes('z.moving = snapshotMoving')) errors.push('v235.9.3 Horde movement/animation state is not synchronized to follower clients.');
// v235.9.4 Horde Nightfall atmosphere uses existing authored night + lighting layers.
if (!runtimeSource.includes('const HORDE_NIGHTFALL=') || !runtimeSource.includes('currentTownNightAlpha=Math.max(getTownNightAlpha(getSharedTownTimeMs()),hordeNightfallAlpha)')) errors.push('v235.9.4 Horde event does not force the outdoor town into night.');
if (!runtimeSource.includes('function drawHordeVisionDarkness') || !runtimeSource.includes('HORDE_NIGHTFALL.visionOuter')) errors.push('v235.9.4 Horde local vision-radius darkness is missing.');
if (!runtimeSource.includes('function getHordeStreetLightAlpha') || !runtimeSource.includes("drawTownLightOverlay(ctx,getHordeStreetLightAlpha(getSharedTownTimeMs()))")) errors.push('v235.9.4 synchronized street-light flicker is missing.');
// v235.10 You Are ATM NFT entitlement + store-lock foundation.
...
if (!html.includes('id="attributeStoreButton"') || !html.includes('id="attributeStorePanel"') || !html.includes('id="attributeStoreCharacters"') || !html.includes('ATTRIBUTE STORE v2 · v235.12.5')) errors.push('v235.12.5 Attribute Store UI markers are missing.');
...
if (!html.includes('mobile Attribute Store layout rebuild') || !html.includes('.attributeStoreBody{\n    position:relative!important;\n    flex:1 1 auto!important;\n    display:flex!important;\n    flex-direction:column!important;') || !html.includes('.attributeStoreGrid{\n    flex:1 1 auto!important;\n    display:flex!important;\n    flex-direction:column!important;') || !html.includes('.attributeStoreCart{\n    position:static!important;') || !html.includes('grid-template-columns:86px minmax(0,1fr)!important;')) errors.push('v235.12 mobile Store flex layout rebuild is missing.');
if (!gameRuntimeParts[0].includes('function attributeStoreCheckoutOpen()') || !gameRuntimeParts[0].includes('function attributeStoreCheckoutClose()') || !gameRuntimeParts[0].includes('attributeStoreState={open:false,checkoutOpen:false')) errors.push('v235.12 checkout sheet runtime state is incomplete.');
...
if (!html.includes('id="attributeStoreCloseButton"') || !html.includes('position:absolute;top:10px;right:10px') || !html.includes('aria-label="Close Attribute Store"')) errors.push('v235.12 persistent Attribute Store close control is missing.');
if (!runtimeSource.includes('VENDING_OPEN_GESTURE_GUARD_MS=850') || !runtimeSource.includes('performance.now()-vendingOpenedAt<VENDING_OPEN_GESTURE_GUARD_MS')) errors.push('v235.11.5 vending open/close gesture guard is missing.');
if (!runtimeSource.includes('let townEntryActive=false') || !runtimeSource.includes('let passkeySignInInProgress=false') || !runtimeSource.includes('if(townEntryActive){hideTownAccessFlow();return;}')) errors.push('v235.11.5 access-flow reentry guard is missing.');
if (runtimeSource.includes("joinOnlineButton.addEventListener('pointerup'")) errors.push('v235.11.5 duplicate touch Enter Town binding is still present.');
if (!pwaSource.includes('serviceWorkerUpdateReady=true') || pwaSource.includes('global.location.reload()')) errors.push('v235.11.5 service-worker live-session reload guard is missing.');
if (!configSource.includes("id: 'cash'") || !configSource.includes("id: 'crypto'") || !configSource.includes("id: 'atm'") || !configSource.includes("id: 'rlusd'") || !configSource.includes("id: 'xrp'") || !configSource.includes("purchaseNetwork: 'mainnet'") || !configSource.includes("merchantAddress: 'rMSDXpxDpV2pQJDHbp77XHHhT9QHMrfPYB'") || !configSource.includes("rMxCKbEDwqr76QuheSUMdEGf4B9xJ8m5De")) errors.push('v235.12 Attribute Store CASH/CRYPTO Mainnet policy is missing.');

if (html.includes('id="attributeStoreShow"') || gameRuntimeParts[0].includes('attributeStoreState.show') || gameRuntimeParts[0].includes('attributeStoreShow')) errors.push('v235.12 must remove the redundant BUYABLE/ALL/OWNED Store selector.');
if (!gameRuntimeParts[0].includes("info.source==='purchase'?'PURCHASED'") || !gameRuntimeParts[0].includes("action.textContent=info.owned?'OWNED ✓'")) errors.push('v235.12 Store cards must label owned/purchased attributes in place.');
if (!configSource.includes('checkoutEnabled: true') || !gameRuntimeParts[0].includes("/api/xaman-vending-start?commerce=attribute-store") || !gameRuntimeParts[0].includes("/api/xaman-vending-status?commerce=attribute-store")) errors.push('v235.12 Attribute Store checkout client routing is missing.');
const attributeStoreLibSource = await readFile(path.join(root, 'lib', 'attribute-store.js'), 'utf8');
const attributeStoreSqlSource = await readFile(path.join(root, 'supabase', 'ATM-Town-v235.12-Attribute-Commerce.sql'), 'utf8');
const vendingStartSource = await readFile(path.join(root, 'api', 'xaman-vending-start.js'), 'utf8');
const vendingStatusSource = await readFile(path.join(root, 'api', 'xaman-vending-status.js'), 'utf8');
if (!attributeStoreLibSource.includes("force_network:'MAINNET'") || !attributeStoreLibSource.includes("ATTRIBUTE_STORE_DESTINATION = 'rMSDXpxDpV2pQJDHbp77XHHhT9QHMrfPYB'") || !attributeStoreLibSource.includes('ensureAttributeEntitlements') || !attributeStoreLibSource.includes('deliveredAmount')) errors.push('v235.12 server-verified Mainnet Attribute Store payment validation is incomplete.');
if (!vendingStartSource.includes("commerce || '').toLowerCase() === 'attribute-store'") || !vendingStatusSource.includes("commerce || '').toLowerCase() === 'attribute-store'")) errors.push('v235.12 Attribute Store must share the existing vending API routes to preserve the 12-function Vercel budget.');
if (!attributeStoreSqlSource.includes('attribute_store_prices') || !attributeStoreSqlSource.includes('attribute_purchase_requests') || !attributeStoreSqlSource.includes('attribute_entitlements') || !attributeStoreSqlSource.includes('enable row level security')) errors.push('v235.12 Attribute Store commerce SQL is incomplete.');
if (!gameRuntimeParts[0].includes("id:'atmNeonRacer',type:'neon-racer'") || !html.includes('id="neonRacerPanel"') || !gameRuntimeSource.includes('global.openATMNeonRacer=open')) errors.push('v235.12 ATM Neon Racer is not wired to the previously unused Racing Cabinet.');
if (!gameRuntimeParts[0].includes("'neon-racer':{panel:'neonRacerPanel',label:'NEON RACER'}") || !gameRuntimeParts[0].includes("'neon-racer':'neonRacerStart'")) errors.push('v235.12 Neon Racer presence/controller integration is missing.');
...
if (!gameRuntimeParts[0].includes('suppressShadow=false') || !gameRuntimeParts[0].includes('downed?null:activity,downed') || !gameRuntimeParts[0].includes('if(!suppressShadow)')) errors.push('v235.12.2 regression: downed-player shadow suppression is missing.');
if (!zombieOutbreakSource.includes('Inferno is a vending power, not a Horde-only visual') || !zombieOutbreakSource.includes("if(player.fireActive&&!player.downed)")) errors.push('v235.12.2 regression: Inferno must render outside The Horde.');
if (!gameRuntimeParts[0].includes('Remote invisibility is complete') || !gameRuntimeParts[0].includes('if(remoteInvisible)continue') || !gameRuntimeParts[0].includes('_invisibleStepCarry') || !gameRuntimeParts[0].includes("spawnFootstepEffect(p.drawX,p.drawY")) errors.push('v235.12.2 regression: remote Invisibility body suppression/footprints are incomplete.');
if (!liveChatSource.includes("TEXT_SIZE_KEY = 'atm_live_chat_text_size_v1'") || !liveChatSource.includes('data-live-chat-text-size') || !html.includes('id="liveChatSettingsButton"') || !html.includes('id="liveChatSettings"') || !html.includes('data-text-size="xlarge"')) errors.push('v235.12.2 regression: Live Chat text-size settings are incomplete.');
if (!gameRuntimeSource.includes('MOBILE_VIEW=Object.freeze({w:560,h:820})') || !gameRuntimeSource.includes('DESKTOP_VIEW=Object.freeze({w:900,h:520})') || !html.includes('#neonRacerCanvas[data-orientation="portrait"]{aspect-ratio:560/820}') || !html.includes('width:auto!important;height:auto!important;max-width:100%!important;max-height:100%!important')) errors.push('v235.12.2 regression: Neon Racer aspect-ratio-preserving mobile layout is missing.');
if (!zombieOutbreakSource.includes('const jumpAmount=Math.max(0,Number(player.jumpAmount??player.jump??0)||0)') || !zombieOutbreakSource.includes('const visualY=y-jumpAmount') || !zombieOutbreakSource.includes('const fy=visualY+25')) errors.push('v235.12.2 Inferno flight anchoring is missing.');
if (!gameRuntimeParts[0].includes('jumpAmount:item.p.jump||0') || !gameRuntimeParts[0].includes('jumpAmount:jumpLift()')) errors.push('v235.12.2 local/remote Inferno lift propagation is missing.');
if (!gameRuntimeParts[0].includes('let bx=b.x,by=b.y,bubbleLift=0') || !gameRuntimeParts[0].includes('const bubbleY=by-bubbleLift') || !gameRuntimeParts[0].includes('bubbleY-78-height')) errors.push('v235.12.2 airborne chat-bubble anchoring is missing.');
...
if (!worldEventsServerSource.includes("sync_model: 'supabase_realtime_elected_authority'")) errors.push('v235.8 combat preview synchronization boundary is missing from server event state.');
...
// v235.8.1 Zombie Outbreak synchronization hotfix.
if (!worldEventsClientSource.includes("atm:world-event-triggered") || !worldEventsClientSource.includes("atm:world-event-state") || !worldEventsClientSource.includes("pageshow")) errors.push('v235.8.1 World Event client refresh/broadcast seam is incomplete.');
if (!zombieOutbreakSource.includes('syncFromWorldEvents') || !zombieOutbreakSource.includes('getBroadcastState') || !zombieOutbreakSource.includes('drawRemoteWeapon')) errors.push('v235.8.1 zombie late-join bootstrap or remote weapon rendering is missing.');
...
if (!zombieOutbreakSource.includes('function refreshAuthority()') || !zombieOutbreakSource.includes("kind: 'snapshot'") || !zombieOutbreakSource.includes('function receiveNetwork(payload = {})')) errors.push('v235.9 elected authority/shared zombie snapshot model is incomplete.');
if (!zombieOutbreakSource.includes("kind !== 'fire'") || !zombieOutbreakSource.includes('applyWeaponShot(weapon, shot, isAuthority())') || !zombieOutbreakSource.includes('flushRapidNetwork(false)')) errors.push('v235.9 shared weapon-fire synchronization is incomplete.');
...
try {
  ...
  const syntaxTargets = [
    ...
    'lib/auth.js', 'lib/live-chat.js', 'lib/xaman-vending.js', 'lib/attribute-store.js', 'lib/xrpl-nft-trading.js', 'lib/atm-pay.js', 'lib/xrpl-testnet-rpc.js', 'lib/world-events.js', 'lib/world-event-money-rain-points.js', 'lib/payload-integration.js', 'lib/payload-money-rain.js',
...
if (errors.length) {
  console.error(`ATM Town v235.12.6 build validation failed with ${errors.length} issue(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log('ATM Town v235.12.6 build validation passed.');
