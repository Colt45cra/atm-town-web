...
function propHuntWinnerId(config = {}) {
  ...
  if (remaining.length === 1) return String(remaining[0].session_id);
  if (!remaining.length) {
...
export async function tagPropHuntPlayer(admin, user, body = {}) {
  ...
  const hunterSessionId = String(body.hunter_session_id || '').trim();
  const targetSessionId = String(body.target_session_id || '').trim();
  const map = String(body.map || '');
  if (!hunterSessionId || !targetSessionId) throw badRequest('Prop Hunt target request is invalid.');
  ...
  if (!participantIds.has(hunterSessionId) || !participantIds.has(targetSessionId)) throw conflict('One of those players is not in this Prop Hunt round.');
  if (String(config.hunter_session_id || '') !== hunterSessionId) throw forbidden('Only the chosen hunter can tag prop players.');
  if (targetSessionId === hunterSessionId) throw conflict('The hunter cannot tag themselves.');
  const foundSessionIds = Array.isArray(config.found_session_ids) ? config.found_session_ids.map((value) => String(value || '')) : [];
  ...
  const remainingProps = participants.filter((entry) => {
    ...
    return sessionId && sessionId !== hunterSessionId && !updatedFound.includes(sessionId);
  });
  let winnerSessionId = '';
  ...
  if (remainingProps.length === 1) {
    winnerSessionId = String(remainingProps[0].session_id || '');
