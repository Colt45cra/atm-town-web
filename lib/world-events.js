...
export function buildZombieOutbreakManifest(seed) {
  ...
  for (const point of pool) {
    ...
    zombies.push({
      ...
      x: point[0],
      y: point[1],
      hp: 10 + Math.floor(random() * 5),
      speed: 48 + Math.floor(random() * 22),
      points: 10,
...
function publicEvent(row, claims, leaders, userId = null) {
  ...
  if (row.event_type === ZOMBIE_EVENT_TYPE) {
    return {
      ...
      ends_at: row.ends_at,
      combat_preview: true,
      sync_model: 'server_timeline_client_combat',
...
export const WORLD_EVENT_ACTIONS = Object.freeze(new Set(['start-money-rain', 'claim-money-rain', 'start-zombie-outbreak']));
