...
const PROP_HUNT_PROP_TYPES = Object.freeze(['town_directory', 'token_market_board', 'atm_vend_blue', 'atm_vend_green', 'street_lamp', 'bench']);
...
function publicEvent(row, claims, leaders, userId = null) {
  ...
  if (row.event_type === PROP_HUNT_EVENT_TYPE) {
    ...
    return {
      ...
      prop_assignment_version: Number(config.prop_assignment_version || 1),
...
export async function getWorldEventState(admin, userId = null) {
  ...
  // The Horde is live-state only. Once its authoritative end time has passed,
  // never return it to clients during reload/focus/reconnect bootstrap. This
  // prevents a completed Horde from being rehydrated and appearing to start
  // without a fresh START THE HORDE request.
  const hordeLive = row.event_type !== ZOMBIE_EVENT_TYPE || eventPhase(row, now) !== 'completed';
  return { server_time_ms: Date.now(), event: addressable && hordeLive ? publicEvent(row, claims, leaders, userId) : null };
}
...
async function insertMoneyRain(admin, user, body = {}, settlement = null) {
  ...
  const config = {
    ...
    ...(settlement ? {
      reward_asset: settlement.asset?.type === 'xrp' ? 'XRP' : String(settlement.asset?.currency || 'TOKEN'),
      reward_asset_detail: settlement.asset,
      reward_network: String(settlement.network || 'mainnet'),
      reward_pool_amount: String(settlement.pool_amount || settlement.pool_xrp),
      reward_pool_xrp: settlement.asset?.type === 'xrp' ? String(settlement.pool_amount || settlement.pool_xrp) : null,
      reward_point_drops: settlement.asset?.type === 'xrp' ? String(settlement.point_drops || '') : null,
      payload: {
...
export async function startFundedMoneyRain(admin, user, body = {}, settlement = {}) {
  ...
  const poolAmount = String(settlement.pool_amount || settlement.pool_xrp || '');
  if (!/^\d+(?:\.\d{1,12})?$/.test(poolAmount) || Number(poolAmount) <= 0 || !settlement.asset) {
    throw badRequest('Payload-funded Money Rain reward amount or asset is invalid.');
  }
...
export async function claimMoneyPickup(admin, user, body = {}) {
  ...
  if (row.config?.reward_settlement) {
    const eligible = await bestEffortRegisterMoneyRainParticipant(admin, user.id, row.config);
    if (!eligible) throw conflict('A verified Xaman wallet with the required Mainnet token trustline is required to collect this Money Rain.');
  }
