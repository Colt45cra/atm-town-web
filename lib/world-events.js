...
async function readRecentEvent(admin) {
  ...
  if (error) throw setupError(error);
  if (!data) return null;
  const endedAt = Date.parse(data.ends_at);
  const settlementStatus = String(data.config?.payload?.settlement_status || '');
  const settlementPending = Boolean(data.config?.reward_settlement) && !['completed', 'cancelled'].includes(settlementStatus);
  // Preview/results normally leave the HUD after 25 seconds. A funded event stays
  // addressable until Payload finishes/refunds so a slow XRPL settlement can never
  // be abandoned just because the presentation results window elapsed.
  if (!settlementPending && Number.isFinite(endedAt) && endedAt < now - RESULTS_WINDOW_MS) return null;
...
export async function getWorldEventState(admin, userId = null) {
  ...
  if (eventPhase(row, Date.now()) === 'completed' && row.config?.reward_settlement) {
