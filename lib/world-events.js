...
export async function getWorldEventState(admin, userId = null) {
  ...
  const now = Date.now();
  if (eventPhase(row, now) === 'completed' && row.config?.reward_settlement) {
  ...
  const backgroundPending = Boolean(row.config?.reward_settlement) && (!settlementFinal || !retirementFinal);
