...
async function rpc(method, params) {
  ...
  try {
    ...
    if (error?.name === 'TimeoutError' || error?.name === 'AbortError') throw new Error('XRPL Testnet verification timed out.');
    throw error;
  }
  if (!response.ok) throw new Error(`XRPL Testnet verification failed (${response.status}).`);
  const json = await response.json();
  if (json?.result?.status === 'error') {
    const err = new Error(json.result.error_message || json.result.error || 'XRPL Testnet verification failed.');
...
async function activity(admin, user) {
  ...
  for (const row of intents || []) {
    ...
    let other = { user_id: row.recipient_user_id, handle: row.recipient_handle, display_name: row.recipient_display_name, character_id: 'classic', atm_pay_ready: true };
    if (!sent) {
      const sender = await identity(admin, row.sender_user_id);
      other = publicIdentity(sender);
    }
    items.push({
...
export const ATM_PAY_ACTIONS = new Set(['pay-status','pay-search','pay-activity','pay-claim-handle','pay-prepare','pay-verify','pay-cancel-intent','pay-submitted','pay-complete','pay-request','pay-request-action']);
