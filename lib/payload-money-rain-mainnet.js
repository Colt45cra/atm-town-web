...
function xrpToDropsExact(value){
  ...
  if(!match) throw bad('XRP funding amount is invalid.');
  const fraction=match[2]||'';
  // Payload may calculate reserve/fee buffers at higher precision than XRP can
  // represent. Round UP to the nearest drop so the disposable wallet is never
  // underfunded by a fractional drop.
  const firstSix=(fraction+'000000').slice(0,6);
  const remainder=fraction.slice(6);
  let drops=BigInt(match[1])*1_000_000n+BigInt(firstSix);
  if(remainder && /[1-9]/.test(remainder)) drops+=1n;
  if(drops<=0n) throw bad('XRP funding amount must be greater than zero.');
