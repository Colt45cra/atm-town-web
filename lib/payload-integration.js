...
function getConfig() {
  ...
  const vercelBypassSecret = String(process.env.PAYLOAD_VERCEL_BYPASS_SECRET || '').trim();
  ...
  return { baseUrl, privateKeyB64, vercelBypassSecret };
}
...
export async function payloadIntegrationRequest(path, body = {}, options = {}) {
  const { baseUrl, vercelBypassSecret } = getConfig();
  ...
  // Payload verifies only URL.pathname. Query parameters are transported in the
  // request URL but are intentionally excluded from the integration signature.
  const signedPath = new URL(url).pathname;
  const message = signaturePayload(method, signedPath, timestamp, nonce, rawBody);
  ...
  if (!response.ok) {
    ...
    else if (data?.protection?.vercel_auth_enabled) {
      message = 'Payload is protected by Vercel Authentication. Configure the ATM Town Payload automation bypass secret.';
    } else if (typeof data?.error?.code === 'string') message = data.error.code;
    if (!message) message = `Payload request failed (${response.status}).`;
    throw requestError(message, response.status, data);
  }
