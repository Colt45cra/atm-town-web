...
import { assertMainnetMoneyRainRecipient } from './payload-money-rain-mainnet.js';
import {
...
export async function bestEffortRegisterMoneyRainParticipant(admin, userId, eventConfig) {
  try {
    ...
    const asset = eventConfig.reward_asset_detail || { type: 'xrp' };
    const walletAddress = eventConfig.reward_network === 'mainnet'
      ? await assertMainnetMoneyRainRecipient(admin, userId, asset)
      : (await readEmbeddedWallet(admin, userId))?.address;
    ...
    await payloadIntegrationRequest(
      ...
      { participants: [{ walletAddress, externalUserId: userId, metadata: { source: 'money_rain_claim' } }] },
    );
    return true;
  } catch (error) {
...
function decimalAmountForPoints(points, poolAmount) {
  const text = String(poolAmount || '').trim();
  if (!/^\d+(?:\.\d{1,12})?$/.test(text)) throw conflict('Money Rain pool amount is invalid.');
  const [whole, fraction = ''] = text.split('.');
  const scale = 10n ** BigInt(fraction.length);
  const units = BigInt(whole) * scale + BigInt(fraction || '0');
  const awarded = units * BigInt(Number(points || 0)) / POINT_POOL;
  const outWhole = awarded / scale;
  const outFraction = (awarded % scale).toString().padStart(fraction.length, '0').replace(/0+$/, '');
  return outFraction ? `${outWhole}.${outFraction}` : String(outWhole);
}
function moneyRainAmountForPoints(points, config) {
  if (config.reward_asset_detail?.type === 'iou') return decimalAmountForPoints(points, config.reward_pool_amount);
  // Mainnet Payload Money Rain stores the authoritative pool amount + points,
  // not the legacy Testnet point_drops field. Divide the funded pool directly.
  if (String(config.reward_network || '').toLowerCase() === 'mainnet') {
    const poolAmount = String(config.reward_pool_amount || config.reward_pool_xrp || '');
    const text = poolAmount.trim();
    if (!/^\d+(?:\.\d{1,12})?$/.test(text)) throw conflict('Money Rain pool amount is invalid.');
    const [whole, fraction = ''] = text.split('.');
    const scale = 10n ** BigInt(fraction.length);
    const poolUnits = BigInt(whole) * scale + BigInt(fraction || '0');
    const pointCount = BigInt(Number(points || 0));
    // XRP is paid in drops. Calculate from the funded pool, then round a
    // positive fractional-drop award UP so a real pickup can never become 0.
    const numerator = poolUnits * 1_000_000n * pointCount;
    const denominator = scale * POINT_POOL;
    const drops = numerator / denominator + (numerator % denominator === 0n ? 0n : 1n);
    return dropsToXrp(drops);
  }
  return dropsToXrp(BigInt(Number(points || 0)) * BigInt(String(config.reward_point_drops || '0')));
}

function cleanupSettlementPatch(cleanup) {
  const immediate = cleanup?.immediateRefund && typeof cleanup.immediateRefund === 'object' ? cleanup.immediateRefund : {};
  const retirement = cleanup?.retirement && typeof cleanup.retirement === 'object' ? cleanup.retirement : {};
  const immediateAmount = immediate.amountXrp ?? immediate.amount;
  return {
    ...(immediateAmount !== undefined ? { immediate_refund_amount: String(immediateAmount) } : {}),
    ...(immediate.txHash ? { immediate_refund_tx_hash: String(immediate.txHash).toUpperCase() } : {}),
    ...(retirement.status ? { retirement_status: String(retirement.status) } : {}),
    ...(Number.isSafeInteger(Number(retirement.eligibleLedger)) ? { retirement_eligible_ledger: Number(retirement.eligibleLedger) } : {}),
    ...(retirement.feeXrp !== undefined ? { retirement_fee_amount: String(retirement.feeXrp) } : {}),
    ...(retirement.recoveredXrp !== undefined ? { retirement_recovered_amount: String(retirement.recoveredXrp) } : {}),
    ...(retirement.txHash ? { retirement_tx_hash: String(retirement.txHash).toUpperCase() } : {}),
    ...(retirement.error ? { retirement_error: String(retirement.error).slice(0, 500) } : { retirement_error: null }),
    ...(retirement.retiredAt ? { retired_at: String(retirement.retiredAt) } : {}),
  };
}

function retirementFinished(payload) {
  return ['retired', 'blocked'].includes(String(payload?.retirement_status || payload?.status || ''));
}

async function advancePayloadRetirement(admin, row, campaignId) {
  const response = await payloadIntegrationRequest(
    `/api/integrations/v1/campaigns/${encodeURIComponent(campaignId)}/retirement`,
    {},
    { timeoutMs: 20_000 },
  );
  const cleanup = response?.cleanup || {};
  const retirement = cleanup?.retirement || {};
  await updateSettlementConfig(admin, row, {
    ...cleanupSettlementPatch(cleanup),
    next_action_at: ['retired', 'blocked'].includes(String(retirement.status || ''))
      ? 0
      : Date.now() + Number(response?.retryAfterMs || retirement?.retryAfterMs || 4_000),
  });
  return row;
}
...
export async function settleCompletedMoneyRain(admin, row, claims, leaders) {
  ...
  const settlementStatus = String(payload.settlement_status || '');
  ...
  try {
    // Player payout/cancellation is final independently from the delayed reserve
    // recovery. Once settlement is done, keep advancing only the campaign-wallet
    // retirement state until AccountDelete succeeds or Payload deliberately blocks.
    if (['completed', 'cancelled'].includes(settlementStatus)) {
      if (retirementFinished(payload)) return row;
      return advancePayloadRetirement(admin, row, campaignId);
    }

    // Read Payload first so a timeout/crash after results were locked cannot make
    // ATM Town try to mutate participants again on the next poll.
    const campaignResponse = await payloadIntegrationRequest(
      `/api/integrations/v1/campaigns/${encodeURIComponent(campaignId)}`,
      null,
      { method: 'GET', timeoutMs: 12_000 },
    );
    const campaignState = campaignResponse?.state || {};
    const remoteStatus = String(campaignState.status || '');
    if (remoteStatus === 'completed' || remoteStatus === 'cancelled') {
      const finalStatus = remoteStatus === 'cancelled' ? 'cancelled' : 'completed';
      await updateSettlementConfig(admin, row, {
        settlement_status: finalStatus,
        settlement_error: null,
        ...cleanupSettlementPatch(campaignState.cleanup),
        next_action_at: 0,
      });
      const refreshedPayload = row.config?.payload || {};
      if (!retirementFinished(refreshedPayload)) return advancePayloadRetirement(admin, row, campaignId);
      return row;
    }
    ...
    if (!claims.length || !leaders.length) {
      ...
      const cleanup = cancelled?.cleanup || {};
      await updateSettlementConfig(admin, row, {
        settlement_status: cancelled?.cancelled ? 'cancelled' : 'refund_pending',
        settlement_error: null,
        ...cleanupSettlementPatch(cleanup),
        next_action_at: retirementFinished(cleanup?.retirement || {}) ? 0 : Date.now() + Number(cleanup?.retirement?.retryAfterMs || 4_000),
      });
      return row;
    }

    // Only a still-funded campaign needs participant registration + immutable
    // result finalization. results_locked/executing/failed campaigns skip
    // directly to the idempotent execute/reconciliation endpoint.
    if (!['results_locked', 'executing', 'failed'].includes(remoteStatus)) {
      const userIds = leaders.map((leader) => leader.user_id);
      const asset = config.reward_asset_detail || { type: 'xrp' };
      const walletEntries = await Promise.all(userIds.map(async (userId) => [
        String(userId),
        config.reward_network === 'mainnet'
          ? await assertMainnetMoneyRainRecipient(admin, userId, asset)
          : (await readEmbeddedWallet(admin, userId))?.address || null,
      ]));
      const walletMap = new Map(walletEntries.filter(([, address]) => CLASSIC_ADDRESS_RE.test(String(address || ''))));
      const missing = userIds.filter((userId) => !walletMap.has(String(userId)));
      if (missing.length) {
        await updateSettlementConfig(admin, row, retryPatch('blocked', `${missing.length} participant wallet(s) lack an eligible verified Xaman wallet or required token trustline.`, 10_000));
        return row;
      }
      ...
      const results = leaders.map((leader) => ({
        walletAddress: walletMap.get(String(leader.user_id)),
        amount: moneyRainAmountForPoints(leader.points, config),
        externalUserId: String(leader.user_id),
      ...
      const canonicalResultPackage = {
        ...
        kind: 'atm_town_money_rain',
        network: String(config.reward_network || 'testnet'),
        asset,
        poolAmount: String(config.reward_pool_amount || config.reward_pool_xrp),
      ...
      await updateSettlementConfig(admin, row, {
        settlement_status: 'results_locked',
        settlement_error: null,
    ...
    const cleanup = execute?.cleanup || {};
    await updateSettlementConfig(admin, row, {
      settlement_status: complete ? 'completed' : String(execute?.campaignStatus || 'executing'),
      settlement_error: null,
      ...cleanupSettlementPatch(cleanup),
      next_action_at: complete
        ? (retirementFinished(cleanup?.retirement || {}) ? 0 : Date.now() + Number(cleanup?.retirement?.retryAfterMs || 4_000))
        : Date.now() + Number(execute?.retryAfterMs || 3_000),
      payout_progress: execute?.progress || null,
    });
    return row;
  } catch (error) {
    console.error('[payload-money-rain] settlement step failed:', error);
    // A payout already known complete must never be visually downgraded just
    // because reserve-recovery polling had a temporary error.
    const currentStatus = String(row?.config?.payload?.settlement_status || settlementStatus || '');
    if (['completed', 'cancelled'].includes(currentStatus)) {
      await updateSettlementConfig(admin, row, {
        retirement_error: String(error?.message || 'Payload reserve recovery failed.').slice(0, 500),
        next_action_at: Date.now() + 8_000,
      });
    } else {
