...
(function installAtmNftPerformance(global){
  ...
  // An IPFS gateway can leave an <img> request pending without firing error.
  // Advance to another candidate after a short timeout instead of displaying a
  // permanent loading icon above the character.
  tradeBeaconSetImage=function tradeBeaconSetImageWithTimeout(host,candidates,alt='XRPL NFT'){
    ...
    img.alt=alt;img.referrerPolicy='no-referrer';img.loading='eager';img.decoding='async';host.appendChild(img);
    const tryNext=()=>{
      if(settled)return;
      clearTimer();
      ...
      img.src=src;
      timer=setTimeout(()=>{if(!settled)tryNext();},BEACON_IMAGE_TIMEOUT_MS);
    };
    img.addEventListener('load',()=>{settled=true;clearTimer();},{once:true});
    img.addEventListener('error',()=>{if(!settled)tryNext();});
    tryNext();
  };
