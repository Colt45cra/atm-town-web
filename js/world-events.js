(function initializeATMWorldEvents(global) {
  ...
  function pickupFrame(pickup, event = state.event) {
    ...
    if (now < landedAt) {
      ...
      const drift = Number(pickup.drift_px || 8);
      return {
        falling: true,
        landed: false,
        x: Number(pickup.x) + Math.sin((now + pickup.id * 97) / 180) * drift * (1 - p),
        y: Number(pickup.y) - Number(pickup.fall_height || 280) * (1 - eased),
        progress: p,
      };
    }
