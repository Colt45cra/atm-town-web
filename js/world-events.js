(function initializeATMWorldEvents(global) {
  ...
  function installUi() {
    ...
    style.textContent = `
      .atmWorldEventHud{position:fixed;z-index:86;top:max(var(--hud-notice-top,74px),calc(env(safe-area-inset-top,0px) + 58px));left:50%;transform:translateX(-50%) translateY(-8px);width:min(560px,calc(100vw - 24px));pointer-events:none;opacity:0;transition:.2s ease;filter:drop-shadow(0 12px 28px rgba(0,0,0,.38));font-family:system-ui,-apple-system,sans-serif}
  ...
  function renderHud() {
    ...
    if (isZombieEvent(event)) {
      ...
      if (phase === 'announced') {
        title.textContent = `THE HORDE IN ${secondsLabel(starts - now)}`;
        meta.textContent = `Triggered by ${providedBy} · get outside · combat preview`;
        score.textContent = 'GET READY';
      } else if (phase === 'active') {
        title.textContent = `THE HORDE · ${secondsLabel(ends - now)} LEFT`;
        const weapon = stats.weaponMode === 'rapid' ? 'RAPID MICRO' : stats.weaponMode === 'spread' ? 'SPREAD' : 'BLASTER';
        meta.textContent = `${Number(event.zombie_count || event.zombies?.length || 0)} horde enemies · ${weapon}`;
        score.textContent = `KILLS ${Number(stats.kills || 0)}`;
      } else {
        title.textContent = 'THE HORDE CONTAINED';
        meta.textContent = `The Horde preview complete · triggered by ${providedBy}`;
        score.textContent = `KILLS ${Number(stats.kills || 0)}`;
      }
  ...
  function renderControlPanel() {
    ...
    const draftStage = String(draft?.funding_stage || draft?.state?.funding?.stage || '');
    const draftRequired = draftStage === 'xrp'
      ? String(draft?.state?.funding?.xrpRequired || draft?.xrp_required || '')
      : String(draft?.state?.funding?.depositRequired || draft?.funding_required || draft?.pool_amount || '');
    ...
    const assetXrp = body.querySelector('#atmWorldEventAssetXrp');
    const assetToken = body.querySelector('#atmWorldEventAssetToken');
    const currencyInput = body.querySelector('#atmWorldEventCurrencyInput');
    const issuerInput = body.querySelector('#atmWorldEventIssuerInput');
    assetXrp?.addEventListener('click', () => { state.moneyRainAssetType = 'xrp'; renderControlPanel(); });
    assetToken?.addEventListener('click', () => { state.moneyRainAssetType = 'iou'; renderControlPanel(); });
