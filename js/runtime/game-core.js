...
const ATM_DISPLAY_BUILD=Object.freeze({version:ATM_CONFIG?.build?.version||'v235.11.5',name:ATM_CONFIG?.build?.name||'Mobile Checkout Fit + Entry Stability'});
...
function drawDepthScene(t){
  ...
  if(currentMap==='town'){
    ...
    for(const [id,p] of remotePlayers){
      ...
      items.push({depth:p.drawY+20,type:'remote',p});
    }
    for(const bot of townBots){
...
// ===== v235.11.5 Character Attribute Store payment rails =====
...
function attributeStoreRenderCart(){attributeStoreNormalizeCart();const host=document.getElementById('attributeStoreCartItems'),title=document.getElementById('attributeStoreCartTitle'),totalNode=document.getElementById('attributeStoreCartTotal'),checkout=document.getElementById('attributeStoreCheckoutButton'),note=document.getElementById('attributeStoreCheckoutNote');const items=attributeStoreCartItems();if(title)title.textContent=items.length+' ITEM'+(items.length===1?'':'S');if(host){host.textContent='';if(!items.length){const empty=document.createElement('div');empty.className='attributeStoreCartEmpty';empty.textContent='Your cart is empty. Shop any character catalog and add attributes here.';host.appendChild(empty);}for(const item of items){const row=document.createElement('div');row.className='attributeStoreCartRow';const preview=lockerItemPreview(item);row.innerHTML=preview?`<img alt="${item.name}" src="${preview}"><div><strong>${item.name}</strong><small>${attributeStorePriceText(item)}</small></div>`:`<span class="fallback">${item.emoji||'◈'}</span><div><strong>${item.name}</strong><small>${attributeStorePriceText(item)}</small></div>`;const remove=document.createElement('button');remove.type='button';remove.className='attributeStoreCartRemove';remove.textContent='×';remove.setAttribute('aria-label','Remove '+item.name);remove.addEventListener('click',()=>attributeStoreToggleCart(item));row.appendChild(remove);host.appendChild(row);}}const prices=items.map(attributeStorePrice),allPriced=items.length>0&&prices.every(price=>price!==null);if(totalNode)totalNode.textContent=allPriced?new Intl.NumberFormat(undefined,{style:'currency',currency:'USD'}).format(prices.reduce((sum,value)=>sum+value,0)):'—';const live=ATM_ATTRIBUTE_STORE_CONFIG?.checkoutEnabled===true&&allPriced;if(checkout){checkout.disabled=!live;checkout.textContent=live?'CHECKOUT · '+attributeStoreSelectedPaymentLabel():items.length?'CHECKOUT SETUP PENDING':'CHECKOUT';}if(note)note.textContent=!items.length?'Add an unowned attribute to start a cart.':!allPriced?'Cart works now; prices have not been assigned yet, so no payment can be submitted.':!ATM_ATTRIBUTE_STORE_CONFIG?.checkoutEnabled?'Pricing is ready, but server-side permanent-purchase checkout is not enabled yet.':'Review the selected payment rail before continuing.';attributeStoreUpdateButton();}
function attributeStoreRenderPayments(){const categories=document.getElementById('attributeStorePaymentCategories'),crypto=document.getElementById('attributeStoreCryptoAssets'),cash=document.getElementById('attributeStoreCashInfo');if(categories){categories.textContent='';for(const method of ATM_ATTRIBUTE_STORE_CONFIG.paymentCategories||[]){const button=document.createElement('button');button.type='button';button.className='attributeStorePaymentCategory'+(attributeStoreState.paymentCategory===method.id?' active':'');button.textContent=method.label;button.title=method.rail;button.addEventListener('click',()=>{attributeStoreState.paymentCategory=method.id;attributeStoreRender();});categories.appendChild(button);}}const cryptoActive=attributeStoreState.paymentCategory==='crypto';if(crypto){crypto.hidden=!cryptoActive;crypto.textContent='';for(const asset of ATM_ATTRIBUTE_STORE_CONFIG.cryptoAssets||[]){const button=document.createElement('button');button.type='button';button.className='attributeStoreCryptoAsset'+(attributeStoreState.cryptoAsset===asset.id?' active':'');button.textContent=asset.label;button.title='XRPL '+String(ATM_ATTRIBUTE_STORE_CONFIG.purchaseNetwork||'mainnet').toUpperCase();button.addEventListener('click',()=>{attributeStoreState.cryptoAsset=asset.id;attributeStoreRender();});crypto.appendChild(button);}}if(cash)cash.hidden=cryptoActive;}
...
document.getElementById('attributeStoreClearCart')?.addEventListener('click',()=>{attributeStoreState.cart=[];attributeStorePersistCart();attributeStoreSetStatus('Cart cleared.');attributeStoreRender();});
...
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&attributeStoreState.open){event.preventDefault();attributeStoreClose();}});
