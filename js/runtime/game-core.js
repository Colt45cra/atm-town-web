...
const ATM_DISPLAY_BUILD=Object.freeze({version:ATM_CONFIG?.build?.version||'v235.11',name:ATM_CONFIG?.build?.name||'Attribute Store Foundation'});
...
document.getElementById('closeVending').addEventListener('click',closeVending);
document.getElementById('jetpackTipClose')?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();hideJetpackTip();});
document.getElementById('vendingPanel').addEventListener('click',e=>{if(e.target.id==='vendingPanel'&&performance.now()-vendingOpenedAt>500)closeVending();});
syncJetpackUi();
updatePowerHud(true);
updateMagnetCartUi();
...
// ===== v235.11 Character Attribute Store =====
const ATM_ATTRIBUTE_STORE_CART_KEY='atm_attribute_store_cart_v1';
const ATM_ATTRIBUTE_STORE_CONFIG=ATM_CONFIG?.attributeStore||Object.freeze({baseCurrency:'USD',checkoutEnabled:false,defaultUsdPrice:null,prices:Object.freeze({}),paymentMethods:Object.freeze([{id:'xrp',label:'XRP',rail:'XRPL'},{id:'atm',label:'ATM',rail:'XRPL'},{id:'rlusd',label:'RLUSD',rail:'XRPL'},{id:'usd',label:'USD',rail:'CARD'}])});
let attributeStoreState={open:false,characterId:'classic',filter:'all',show:'buyable',paymentMethod:'xrp',cart:safeJsonParse(safeStorageGet(ATM_ATTRIBUTE_STORE_CART_KEY,'[]'),[])};
...
function attributeStoreRenderCart(){attributeStoreNormalizeCart();const host=document.getElementById('attributeStoreCartItems'),title=document.getElementById('attributeStoreCartTitle'),totalNode=document.getElementById('attributeStoreCartTotal'),checkout=document.getElementById('attributeStoreCheckoutButton'),note=document.getElementById('attributeStoreCheckoutNote');const items=attributeStoreCartItems();if(title)title.textContent=items.length+' ITEM'+(items.length===1?'':'S');if(host){host.textContent='';if(!items.length){const empty=document.createElement('div');empty.className='attributeStoreCartEmpty';empty.textContent='Your cart is empty. Shop any character catalog and add attributes here.';host.appendChild(empty);}for(const item of items){const row=document.createElement('div');row.className='attributeStoreCartRow';const preview=lockerItemPreview(item);row.innerHTML=preview?`<img alt="${item.name}" src="${preview}"><div><strong>${item.name}</strong><small>${attributeStorePriceText(item)}</small></div>`:`<span class="fallback">${item.emoji||'◈'}</span><div><strong>${item.name}</strong><small>${attributeStorePriceText(item)}</small></div>`;const remove=document.createElement('button');remove.type='button';remove.className='attributeStoreCartRemove';remove.textContent='×';remove.setAttribute('aria-label','Remove '+item.name);remove.addEventListener('click',()=>attributeStoreToggleCart(item));row.appendChild(remove);host.appendChild(row);}}const prices=items.map(attributeStorePrice),allPriced=items.length>0&&prices.every(price=>price!==null);if(totalNode)totalNode.textContent=allPriced?new Intl.NumberFormat(undefined,{style:'currency',currency:'USD'}).format(prices.reduce((sum,value)=>sum+value,0)):'—';const live=ATM_ATTRIBUTE_STORE_CONFIG?.checkoutEnabled===true&&allPriced;if(checkout){checkout.disabled=!live;checkout.textContent=live?'CHECKOUT WITH '+String(attributeStoreState.paymentMethod||'xrp').toUpperCase():items.length?'CHECKOUT SETUP PENDING':'CHECKOUT';}if(note)note.textContent=!items.length?'Add an unowned attribute to start a cart.':!allPriced?'Cart works now; prices have not been assigned yet, so no payment can be submitted.':!ATM_ATTRIBUTE_STORE_CONFIG?.checkoutEnabled?'Pricing is ready, but server-side permanent-purchase checkout is not enabled yet.':'Review the selected payment method before continuing.';attributeStoreUpdateButton();}
function attributeStoreRenderPayments(){const host=document.getElementById('attributeStorePaymentMethods');if(!host)return;host.textContent='';for(const method of ATM_ATTRIBUTE_STORE_CONFIG.paymentMethods||[]){const button=document.createElement('button');button.type='button';button.className='attributeStorePaymentMethod'+(attributeStoreState.paymentMethod===method.id?' active':'');button.textContent=method.label;button.title=method.rail;button.addEventListener('click',()=>{attributeStoreState.paymentMethod=method.id;attributeStoreRender();});host.appendChild(button);}}
